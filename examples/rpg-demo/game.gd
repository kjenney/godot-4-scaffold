...
func _ready() -> void:
	...
	for n in $Exploration/Grid.get_children():
		if not n is Grid:
			continue
		if not n.has_node("Player"):
			continue
		var player := n.get_node("Player")
		if not player.has_node("DialoguePlayer"):
			continue
		player.get_node("DialoguePlayer").dialogue_finished.connect(_on_opponent_dialogue_finished.bind(n))
...
func _on_opponent_dialogue_finished(opponent: Node) -> void:
	if opponent.lost:
		return
...
func _on_combat_finished(winner: Node, _loser: Node) -> void:
	remove_child(combat_screen)
	$AnimationPlayer.play_backwards("fade_to_black")
	...
	await $AnimationPlayer.animation_finished
	var player: Node = $Exploration/Grid/Player
